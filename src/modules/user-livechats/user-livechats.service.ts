...
export class UserLivechatsService implements IUserLiveChatService {
  ...
  async getAllUserLiveChats(): Promise<ResData<UserLivechatEntity[]>> {
    ...
    return new ResData<UserLivechatEntity[]>("All accepted live chats", 200, foundUserLiveChats);
  }

  async getUserLiveChatsByUserId(userId: number): Promise<ResData<UserLivechatEntity[]>>{
  ...
  async getUserLiveChatsByTeacherId(teacherId: number): Promise<ResData<UserLivechatEntity[]>>{
    const { data: foundTeacher } = await this.userService.findOneById(teacherId);
    const foundUserLiveChatsByTeacherId = await this.userLiveChatRepository.getByTeacherId(teacherId, foundTeacher.courseId);
